...
from datasets.data_tools import get_labels_bi, np_to_tensor, warpLabels
...
from utils.utils import compute_valid_mask, dict_update, filter_points
...
class SyntheticDataset_gaussian(data.Dataset):
    ...
    def dump_primitive_data(self, primitive, tar_path, config):
        ...
        for split, size in self.config["generation"]["split_sizes"].items():
            ...
            for i in tqdm(range(size), desc=split, leave=False):
                ...
                points = np.array(
                    getattr(synthetic_dataset, primitive)(image, **config["generation"]["params"].get(primitive, {}))
                )
    ...
    def __getitem__(self, index):
        ...
        if (self.config["augmentation"]["photometric"]["enable_train"] and self.action == "training") or (
        ...
        if not (
            ...
            or (self.config["augmentation"]["homographic"]["enable_val"] and self.action == "validation")
        ):
        ...
        if self.config["warped_pair"]["enable"]:
            ...
            if (self.enable_photo_train and self.action == "train") or (self.enable_photo_val and self.action == "val"):
