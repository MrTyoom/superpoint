...
class PixelwiseContrastiveLoss(object):
    ...
    @staticmethod
    def flattened_pixel_locations_to_u_v_static(flat_pixel_locations, image_width):
        """
        Статическая версия метода
        :param flat_pixel_locations: torch.LongTensor shape [n, 1]
        :param image_width: ширина изображения
        :return: torch.LongTensor shape [n, 2]
        """
    ...
    def flattened_pixel_locations_to_u_v(self, flat_pixel_locations):
        ...
        """
        return self.flattened_pixel_locations_to_u_v_static(
            flat_pixel_locations, 
            self.image_width
        )
