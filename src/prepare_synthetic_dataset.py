...
def draw_multiple_polygons(
    image, random_state, max_sides=8, nb_polygons=30, kernel_boundaries=(50, 100)
):
...
def draw_checkerboard(
    image, random_state, max_rows=7, max_cols=7, transform_params=(0.05, 0.15)
):
    ...
    s = min(
        (image.shape[1] - 1) // cols, (image.shape[0] - 1) // rows
    )  # size of a cell
    ...
    alpha_affine = np.max(image.shape) * (
        ...
        for j in range(cols):
            ...
            if i == 0 and j == 0:
                ...
                col = get_different_color(
                    np.array(neighboring_colors), random_state=random_state
                )
    ...
    trans = np.array(
        [
            ...
            + random_state.randint(
                -image.shape[1] * trans_interval[1], image.shape[1] * trans_interval[1]
            ),
            image.shape[0] * trans_interval[0]
            + random_state.randint(
                -image.shape[0] * trans_interval[1], image.shape[0] * trans_interval[1]
            ),
            0,
        ]
    )
