...
def get_different_color(previous_colors, min_dist=50, max_count=20):
    ...
    count = 0
    while (
        np.any(np.abs(previous_colors - color) < min_dist) and count < max_count
    ):  # max_count times check if new_color has enough contrast
        count += 1
        color = random_state.randint(256)
    return color
...
def generate_custom_background(size, background_color, nb_blobs=3000, kernel_boundaries=(50, 100)):
    """Generate a customized background to fill the shapes(more neutral and monotonic)
    Parameters:
...
def draw_stripes(img, max_nb_cols=13, min_width_ratio=0.04, transform_params=(0.05, 0.15)):
...
def draw_cube(
    ...
    trans = np.array(
        [
            ...
            + random_state.randint(-img.shape[1] * trans_interval[1], img.shape[1] * trans_interval[1]),
            img.shape[0] * trans_interval[0]
            + random_state.randint(-img.shape[0] * trans_interval[1], img.shape[0] * trans_interval[1]),
            0,
        ]
    )
