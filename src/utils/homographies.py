...
def sample_homography_np(
    ...
    if rotation:
        ...
        angles = np.concatenate((angles, np.array([0.0])), axis=0)  # in case no rotation is valid
        center = np.mean(pts2, axis=0, keepdims=True)
        rot_mat = np.reshape(
            np.stack(
    ...
    homography = cv2.getPerspectiveTransform(
        np.float32(pts1 + shift),  # output image
...
#         h_displacement_right = tf.truncated_normal(
...
#     if scaling:
#         scales = tf.concat(
#             [[1.0], tf.truncated_normal([n_scales], 1, scaling_amplitude / 2)], 0
#         )
...
#     trans = torch.tensor(
#         [[2.0 / width, 0.0, shift[0]], [0.0, 2.0 / height, shift[1]], [0.0, 0.0, 1.0]],
#         dtype=dtype,
#     )
