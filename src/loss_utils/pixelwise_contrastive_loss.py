...
class PixelwiseContrastiveLoss(object):
    ...
    def get_loss_matched_and_non_matched_with_l2(
        ...
        if use_l2_pixel_loss:
            non_match_loss, num_hard_negatives = self.non_match_loss_with_l2_pixel_norm(
                image_a_pred,
                image_b_pred,
                matches_b,
                non_matches_a,
    ...
    @staticmethod
    def match_loss(
        image_a_pred, image_b_pred, matches_a, matches_b, M=1.0, dist="euclidean", method="1d"
    ):  # dist = 'cos'
        r"""
    ...
    def non_match_descriptor_loss(
        image_a_pred, image_b_pred, non_matches_a, non_matches_b, M=0.5, invert=False, dist="euclidear"
    ):
        """
    ...
    def non_match_loss_with_l2_pixel_norm(
        self, image_a_pred, image_b_pred, matches_b, non_matches_a, non_matches_b, M_descriptor=0.5, M_pixel=None
    ):
        """
    ...
    def non_match_loss_descriptor_only(
        self, image_a_pred, image_b_pred, non_matches_a, non_matches_b, M_descriptor=0.5, invert=False
    ):
        """
        ...
        non_match_loss_vec, num_hard_negatives, _, _ = PCL.non_match_descriptor_loss(
            image_a_pred, image_b_pred, non_matches_a, non_matches_b, M=M_descriptor, invert=invert
        )
    ...
    def l2_pixel_loss(self, matches_b, non_matches_b, M_pixel=None):
        ...
        ground_truth_pixels_for_non_matches_b = (
            torch.t(matches_b.repeat(num_non_matches_per_match, 1)).contiguous().view(-1, 1)
        )
        ...
        squared_l2_pixel_loss = (
            1.0 / M_pixel * torch.clamp((ground_truth_u_v_b - sampled_u_v_b).float().norm(norm_degree, 1), max=M_pixel)
        )
