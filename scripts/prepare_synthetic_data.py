...
def get_different_color(previous_colors, min_dist=50, max_count=20):
    ...
    while (
        np.any(np.abs(previous_colors - color) < min_dist)
        and count < max_count
    ):
        count += 1
        color = random_state.randint(256)
...
def generate_background(
    ...
    cv.threshold(
        image, random_state.randint(256), 255, cv.THRESH_BINARY, image
    )
    ...
    for i in range(nb_blobs):
        ...
        cv.circle(
            ...
            (blobs[i][0], blobs[i][1]),
            np.random.randint(
                int(dim * min_rad_ratio), int(dim * max_rad_ratio)
            ),
            col,
            -1,
        )
...
def generate_custom_background(
    size, background_color, kernel_boundaries=(50, 100), nb_blobs=3000
):
    ...
    for i in range(nb_blobs):
        col = get_random_color(background_color)
        cv.circle(
            img, (blobs[i][0], blobs[i][1]), np.random.randint(20), col, -1
        )
...
def draw_lines(image, nb_lines=10):
    ...
    for _ in range(num_lines):
        ...
        cv.line(image, (x1, y1), (x2, y2), col, thickness)
...
def draw_polygon(image, max_sides=8):
    ...
    if num_corners < 3:  # not enough corners
        return draw_polygon(image, max_sides)
...
def draw_multiple_polygons(
    img, max_sides=8, nb_polygons=30, kernel_boundaries=(50, 100)
):
    segments = np.empty((0, 4), dtype=int)
    centers = []
    rads = []
    ...
    for i in range(nb_polygons):
        ...
        custom_background = generate_custom_background(
            img.shape, background_color, kernel_boundaries
        )
...
def draw_ellipses(img, nb_ellipses=20):
    ...
    return img, np.empty((0, 2), dtype=int)
...
def draw_checkerboard(
    img, max_rows=7, max_cols=7, transform_params=(0.05, 0.15)
):
    ...
    s = min(
        (img.shape[1] - 1) // cols, (img.shape[0] - 1) // rows
    )  # size of a cell
    ...
    warped_col0 = np.add(
        np.sum(
            np.multiply(warped_points, perspective_transform[0, :2]), axis=1
        ),
        perspective_transform[0, 2],
    )
    warped_col1 = np.add(
        np.sum(
            np.multiply(warped_points, perspective_transform[1, :2]), axis=1
        ),
        perspective_transform[1, 2],
    )
    warped_col2 = np.add(
        np.sum(
            np.multiply(warped_points, perspective_transform[2, :2]), axis=1
        ),
...
def draw_stripes(
    img, max_nb_cols=13, min_width_ratio=0.04, transform_params=(0.1, 0.1)
):
    ...
    cols2 = np.concatenate(
        [cols, (board_size[0] - 1) * np.ones((col + 1, 1), int)], axis=1
    )
...
def draw_cube(
    ...
    lx = (
        min_side + random_state.rand() * 2 * min_dim / 3
    ...
    scaling = np.array(
        [
            [
                scale_interval[0] + random_state.rand() * scale_interval[1],
                0,
                0,
            ],
            [
...
def generate_data(n, out_dir, draw_fn, background_size, image_size, blur_size):
    ...
    image = cv2.resize(
        image, (image_size[1], image_size[0]), interpolation=cv2.INTER_LINEAR
    )
    ...
    for name, size in cfg.primitives.items():
        ...
        with parallel_backend("threading"):
            Parallel(n_jobs=n_jobs)(
                generate_data(
                    n, out_dir, draw_fn, background_size, image_size, blur_size
                )
